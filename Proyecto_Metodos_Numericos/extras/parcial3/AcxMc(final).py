...
def formatear_polinomio(coefs):
    ...
    for i, c in enumerate(coefs):
        if i == 0:
            partes.append(f"{c:.6f}")
        elif i == 1:
            partes.append(f"{c:.6f}*x")
        else:
            partes.append(f"{c:.6f}*x^{i}")
    return " + ".join(partes)


def ajuste_por_grado(xs, ys, grado):
    A, b = construir_sistema_normal(xs, ys, grado)
    M_gauss, v_gauss, coefs = gauss_eliminacion_detallado(A, b)
...
def texto_modelo(grado, coefs):
    ...
    for i in range(1, grado + 1):
        if i == 1:
            partes.append(f"({coefs[i]:.6f})X")
        else:
            partes.append(f"({coefs[i]:.6f})X^{i}")
...
class App:
    ...
    def calcular(self):
        ...
        self.escribir(
            ...
            + ", ".join(f"K{i} = {k:.6f}" for i, k in enumerate(resultado_base["coefs"]))
        )
        ...
        for i, x_eval in enumerate(xs_eval, start=1):
            y_eval = evaluar_polinomio(mejor["coefs"], x_eval)
